...
"@(#) faibidnt.f90	V-0.3.1 12/01/09 Michel Olagnon"
...
&"                    version 0.3.1 by F. Reinquin, 04 Jan 2012"/&
&"____________________________________________________________________"/&
...
!
      ...
      integer, parameter  :: nsttcm = 4*(ncntm+1)
                             ! same including embedded comments
...
!
      ...
      integer, parameter  :: kkecp = 40 ! Embedded CPP instr. in continued instr.
      integer, parameter  :: kkcco = 41 ! /*
      integer, parameter  :: kkccf = 42 ! */
      integer, parameter  :: kkukn = 43 ! Other
!   Macros
...
!
      character (len=*), parameter  :: zoshp = "FPPR_C_SHARP"
      integer, save                 :: iwshp
...
! ____________________________________________________________________
      character (len=*), parameter :: zpnam = "f90ppr" 
      ...
      character (len=linem+1), dimension (nsttcm), save :: zsttot
                                  ! original lines for current statement
                                  ! (embedded comments when continuation)
      integer, save   :: nltto = 1 ! How many lines do we have in statement
...
!
      ...
      integer, save     :: ifshp = 1 ! is # used for cpp ?
!
!  Case processing
!
...
! ____________________________________________________________________
!
      write (luerr, "(a)") "This is f90aib: " // zvers
      write (luerr, "(a)")  zusg
!
...
!
      ksta  = 0
      ifccm = 0 ! C type comment
      ifccc = 0 ! continued comments
      ifcnt = 0
      ifchc = 0
      ntok  = 0
      kktok = kkndf
...
!
         ...
         if (llin == 0) then
            ...
            if (ifcnt == 0) then
               ...
               call trttok (zblk, len(zblk), kkebc)
               cycle body
            endif
         endif
         ilin = verify (zlin (1:llin), ztab // " ")
cmtl:    do
! write (0,*) trim (zlin), " : ", ilin, ifccc, ifccm, ifcct
            if (ilin /= 0) then
              if (ifccc == 0) then
               if (zlin (ilin:ilin) /= "!") then
                  if (ifshp == 0)  then
                     exit cmtl
                  elseif (zlin (ilin:ilin) /= "#") then
                     exit cmtl
                  else ! ifshp = 1 and #
                     ifccm = 1
                     if (ifcnt /= 0) ifcnt = 2
...
!
!  It is a comment - check for C type continuation of it
!
            if (ifccm /= 0) then
               if (zlin(llin:llin) == zbks) then
                  ifccc = max (1, ifccc)
               else
                  ideb = 1
embedc:           do
                    if (ifcct == 0) then
                      ilin = index (zlin (ideb:llin),"/*")
                      if (ilin == 0) exit embedc
                      ifcct = 1
                      ideb = ideb + ilin + 1
                      if (ideb > ilin) exit embedc
                    else ! C comment context
                      ilin = index (zlin (ideb:llin),"*/")
                      if (ilin == 0) then
                        ifccc = 2
                        exit embedc
...
!
!  Continuation by backslash at end of line
!
                     case (zbks)
                        if (ichr == llin) ifcnt = 1
!
!  Percent
!
                     case ('%')
...
&OUTFPPR_USE_SHARPFPPR_MAX_LINEFPPR_PGM_INDENTFPPR_STP_INDENTFPPR_NMBR_LINESFPPR_BLANK_CMTFPPR_C_SHARP"
      inamg =   1271
!
      znamo (     1:  1290) = "&
&$$Define$ElIf$Else$EndIf$Eval$If$IfDef$IfnDef$Include$Macro$UnDef.And..Eq..Eqv..False..Ge..Gt..Le..Lt..Ne..Neqv..Not..Or..True.Abs&
...
&PPR_fxd_inFPPR_fxd_outFPPR_use_sharpFPPR_max_lineFPPR_pgm_indentFPPR_stp_indentFPPR_nmbr_linesFPPR_blank_cmtFPPR_C_sharp"
      inamo =   1290
!
      ...
      tnamt ( 2625) = namtyp (    0,    0,  263,  1260,  1271,  1279,  1290)
...
!
! leave instruction as such if CPP commands intermixed
!
      if (ntok > 0) then
         if (any (kktokt (1:ntok) == kkecp) .and. nltto > 1) then
           do iltto = 1, nltto
             zcmt = zsttot (iltto)
             lcmt = len_trim (zcmt)
             call wrtstt (zblk, 0, zblk, 0, zcmt, lcmt, 0)
           enddo
           return
         endif
      endif
...
!
            ...
            case (kkcmt)   ! Comment string
               ...
               lcmt = ltok
               exit
            case (kkebc)   ! Embedded comment string
               if (ifblk == 0) then
                  ltok = ltokw + 1
                  zcmt (1:ltok) = " " // ztokw (1:ltokw)
               else
...
!
           if (any (kktokt (1:ntok) == kkebc) .and. nltto > 1) then
             do iltto = 1, nltto
               ideb = verify (zsttot (iltto), ' '//'!'//ztab)
               ideb = min (ideb, 1+len_trim (zpnam)+1)
               zcmt = '!'//zpnam//zsttot (iltto)(ideb:)
               lcmt = len_trim (zcmt)
               call wrtstt (zblk, 0, zblk, 0, zcmt, lcmt, 0)
             enddo
           endif
...
!
!   Treat Sharp as cpp command
!
                  elseif (inam == iwshp) then
                     itok = 3
                     if (ntok < itok) then
                        ifshp = 1
                        ifsed = 0
