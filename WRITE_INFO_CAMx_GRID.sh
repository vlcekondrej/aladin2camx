...
mez[0]=0
mez[1]=14 # az po tuto hladinu se budou vypisovat po jedne. Pokud chci agregovat uz od prvni, musim zde zadat 0 a nasledny cylkus mezet od 2 namisto 1 
...
for i in $(seq 1 4); do
  for lev in $( seq $((${mez[$i-1]}+1)) $i ${mez[$i]}); do
    lev1=$lev
    lev2=$(($lev+$i-1))
    echo "CAMx_levDef(" $nlev",1) = " $lev1 >> ${CAMx_GRID_NML}
    echo "CAMx_levDef(" $nlev",2) = " $lev2 >> ${CAMx_GRID_NML}
    nlev=$(($nlev+1))
  done
  echo "" >> ${CAMx_GRID_NML}
done

echo "CAMx_levDef(" $nlev",1) = " $(($lev2+1)) >> ${CAMx_GRID_NML}
echo "CAMx_levDef(" $nlev",2) = " $(($lev2+1)) >> ${CAMx_GRID_NML}
